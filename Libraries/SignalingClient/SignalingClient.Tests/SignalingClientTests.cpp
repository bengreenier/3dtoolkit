...
TEST(SignalingClient, TurnCredentialProviderSuccess)
{
	...
	{
		// TODO(bengreenier): send back handshake data
		auto mockSocket = make_unique<MockSslCapableSocket>(a, b, c);
	...
	EXPECT_CALL(mockProvider, Authenticate()).Times(Exactly(1)).WillRepeatedly(Return(true));
	...
	TurnCredentialProvider::CredentialsRetrievedCallback cb([&](const TurnCredentials& result)
	{
		...
		ASSERT_STREQ(result.username.c_str(), "");
		ASSERT_STREQ(result.password.c_str(), "");
...
TEST(SignalingClient, SslCapableSocketPassthrough)
{
	...
	SslCapableSocket socket(move(mockSocket), false, std::weak_ptr<rtc::Thread>(mockThread));
